...
export default class SteroidObject extends Phaser.Physics.Arcade.Sprite {
    ...
    shake(selected, scene) {
        ...
        scene.sound.play(selected ? 'CorrectSound' : 'BadSound' ); 
        if (selected && scene.scene.key !== 'ObjectLoby') {
            // no mas clicks
            obj.active = false; 
        }
        this.scene.tweens.addCounter({
            ...
            onComplete: function () {
                if (self.scene.key == 'ObjectRondas') {
                    if (selected) {
                        ...
                        scene.tries -= 1; 
                        scene.texto_numberintentos.setText('Intentos: ' + scene.tries); 
